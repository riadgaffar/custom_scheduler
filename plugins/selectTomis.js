...
(function () {
  ...
  .service('RepeatParser', ['uiSelectMinErr','$parse', function(uiSelectMinErr, $parse) {
    ...
    self.getNgRepeatExpression = function(itemName, source, trackByExp, grouped, limit) {
  ...
  .controller('uiSelectCtrl',
    ...
    ctrl.minLength = 0; // Initialized inside uiSelectChoices directive link function
    ...
    ctrl.activate = function(initSearchValue, avoidReset) {
      if (!ctrl.disabled  && !ctrl.open && (!ctrl.minLength || ctrl.search.length>=ctrl.minLength)) {
        if(!avoidReset){
        	initSearchValue=undefined;
        	_resetSearchInput();
        }
...
//        $element.find('#spinner').addClass('hidden');
        $element.find('.select-spinner').addClass('hidden');
        $element.find('.select-spinner').removeClass('loading');
        $element.find('.select-caret').removeClass('hidden');
        var remainingSpinners = document.querySelectorAll('.select-spinner.loading');
        if (remainingSpinners.length == 0) {
        	if ($scope.handleFinishedLoadingLovs) {
        		$scope.handleFinishedLoadingLovs();
        	}
        } else {
//        	console.log('select-spinner count=' + remainingSpinners.length);
        }
      }
      ...
      if (item === undefined || !item._uiSelectChoiceDisabled) {
        ...
        if (!item || !item._uiSelectChoiceDisabled) {
          ...
          if(ctrl.multiple) {
            ...
            ctrl.search=EMPTY_SEARCH;  //We may want to reset the search after selecting an item for multi selects otherwise it looks kinda funny.
            ctrl.sizeSearchInput();
          } else {
            ctrl.selected = item;
          }
  ...
  .directive('uiSelect',
    ...
    return {
      ...
      link: function(scope, element, attrs, ctrls, transcludeFn) {
        ...
        element.querySelectorAll('.select-spinner').removeClass('hidden').addClass('loading');
    ...
    return {
      ...
      compile: function(tElement, tAttrs) {
        ...
        return function link(scope, element, attrs, $select, transcludeFn) {
          ...
          choices.attr('ng-repeat', RepeatParser.getNgRepeatExpression($select.parserResult.itemName, '$select.items', $select.parserResult.trackByExp, groupByExp,limit))
          ...
          scope.$watch('$select.search', function(newValue) {
        	if(newValue && newValue.length<$select.minLength){
        		$select.open=false;
        	}else{
          ...
          //Adding a min length attribute specifically for the TOMIS Application
          //TODO In the future, when this is implemented by Ui-Select per .12 release, we will replace this with their version
          //3/24/15
          attrs.$observe('minLength', function() {
              // $eval() is needed otherwise we get a string instead of a number
              var minLength = scope.$eval(attrs.minLength);
              $select.minLength = minLength !== undefined ? minLength : uiSelectConfig.minLength;
          });
        };
      }
    };
...
$templateCache.put("bootstrap/match.tpl.html","<div class=\"ui-select-match\" ng-hide=\"$select.open\" ng-disabled=\"$select.disabled\" ng-class=\"{\'btn-default-focus\':$select.focus}\"><button type=\"button\" class=\"btn btn-default btn-block ui-select-toggle\" tabindex=\"-1\" ;=\"\" ng-disabled=\"$select.disabled\" ng-click=\"$select.activate($select.selected.description, false)\"><span ng-show=\"$select.isEmpty()\" class=\"ui-select-placeholder text-muted\">{{$select.placeholder}}</span> <span ng-hide=\"$select.isEmpty()\" class=\"ui-select-match-text\" ng-class=\"{\'ui-select-allow-clear\': $select.allowClear && !$select.isEmpty()}\" ng-transclude=\"\"></span> <i class=\"caret pull-right\" ng-click=\"$select.toggle($event)\"></i></button> <button type=\"button\" class=\"ui-select-clear btn btn-default\" ng-if=\"$select.allowClear && !$select.isEmpty()\" ng-click=\"$select.select(undefined)\"><i class=\"glyphicon glyphicon-remove\"></i></button></div>");
