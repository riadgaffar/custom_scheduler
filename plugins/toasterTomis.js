    ...
    return {
        ...
        link: function (scope, elm, attrs) {
            ...
            function addToast(toast) {
                ...
                toast.otherClasses = 'new-class1 new-class2';
                if (!toast.type)
                ...
                toast.maxWidthStyle=function maxWidthStyle(toast) {
                	var $toastContainer = angular.element('#toast-container');
                	var fontSizePx = 32; // default 2em
                	//Determine size of text and resize toaster elements accordingly
                	if ($toastContainer.length > 0) {
                		fontSizePx = parseFloat($toastContainer.css('font-size'));
                	}
                	// Set message box max width
                	var widthFactor = fontSizePx*1.5;
                	var maxWidth = toast.data.length*widthFactor;
                	//Set maximum max-width
                	if (maxWidth > 900) {
                		maxWidth = 900;
                	}
                ...
                toast.inlineStyle=function inlineStyle(toast, toasterBodyRowInstanceName) {
                	var $toastContainer = angular.element('#toast-container');
                    //Icon image placement/sizing
                    //angular.element('.toast-container-icon.warning').outerHeight()
                    var $toasterBodyRow = $toastContainer.find('.toaster-body.row');
                    if (toasterBodyRowInstanceName) {
                    	$toasterBodyRow = angular.element('[data-instance="' + toasterBodyRowInstanceName + '"]');
                    }
                    if ($toasterBodyRow.hasClass('resized')) {
                    	return;
                    }
                    $toasterBodyRow.addClass('resized');
                    var $toasterBodyIconCol = $toasterBodyRow.find('section');
                    if ($toasterBodyIconCol.length > 0) {
                        var rowH = $toasterBodyRow.outerHeight();
                        var colW = $toasterBodyIconCol.outerWidth();
                        var $iconContainer = $toasterBodyRow.find('.toast-container-icon');
                        if ($iconContainer.length > 0) {
                        	var fudgeLeftPct = 1;
                        	var iconContainerW = $iconContainer.width();
                        	var iconContainerH = $iconContainer.outerHeight();
                        	
                            var iconContainerOffset = $iconContainer.offset();
                            
                            if (iconContainerW > 500) {
                            	fudgeLeftPct = fudgeLeftPct*0.25;
                            }
                            
                        	var iconContainerLeft = iconContainerOffset.left*fudgeLeftPct;
                        	var iconContainerTop = iconContainerOffset.top;
                        	
                        	// Center icon horizontally and vertically within row.
                        	var newLeft = iconContainerLeft + (colW - iconContainerW)/2;
                        	var newTop = iconContainerTop + (rowH - iconContainerH)/2;
                        	$iconContainer.offset({'left': newLeft, 'top': newTop});
                        	
                        	var fudgeScalePct = 1;
                        	var newScale = rowH*fudgeScalePct/iconContainerW;
                        	if (newScale > 1.75) {
                        		newScale = newScale*0.75;
                        	}
                        	$iconContainer.css({'transform': 'scale('+newScale+')'});
                        }
                    }
                    return {};
                };
        ...
        template:
        '<div id="toast-container" class="toaster-container" ng-class="[config.position, config.animation]">' +
          '<div ng-repeat="toaster in toasters" class="toast" ng-class="[toaster.type, toaster.otherClasses]" ng-click="click(toaster)" ng-mouseover="stopTimer(toaster)" ng-mouseout="restartTimer(toaster)" ng-style="toaster.maxWidthStyle(toaster)">' + 
